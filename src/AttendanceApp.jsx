...
const AttendanceApp = () => {
  ...
  const [subjects, setSubjects] = useState([
    { 
      ...
      attended: 15, 
      dl: 0, 
      schedule: [0, 1, 0, 1, 0, 1, 0] // Default: Mon, Wed, Fri
    }
  ]);

  // --- Persistence (Load) ---
  useEffect(() => {
    const saved = localStorage.getItem('attendanceApp_jsx');
    if (saved) {
      try {
  ...
  // --- Persistence (Save) ---
  useEffect(() => {
    localStorage.setItem('attendanceApp_jsx', JSON.stringify({
  ...
  const handleReset = () => {
    if (window.confirm("Start Fresh? This will delete all current subjects and settings.")) {
      ...
      localStorage.removeItem('attendanceApp_jsx');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-24 p-4 md:p-8">
      
      {/* --- Top Bar --- */}
      <div className="bg-white border-b border-slate-200 sticky top-0 z-20 shadow-sm rounded-xl mb-6">
